package com.funkypear.ui
{
   import flash.display.Sprite;
   import §_-ZN§.VictoryButton;
   
   public class VictorySelectPanel extends Sprite
   {
      
      public function VictorySelectPanel()
      {
         var _loc1_:* = true;
         var _loc2_:* = false;
         _loc2_;
         this.anims = new Array();
         _loc1_;
         super();
         _loc1_;
         this.holder = new Sprite();
         _loc1_;
         addChild(this.holder);
      }
      
      private var §_-sD§:SelectBoxBackground;
      
      private var anims:Array;
      
      private var start_x:Number;
      
      private var start_y:Number;
      
      private var holder:Sprite;
      
      private var §_-jC§:§_-GI§;
      
      public function §_-C1§() : void
      {
         var _loc4_:* = false;
         var _loc5_:* = true;
         var _loc3_:* = 0;
         _loc5_;
         _loc5_;
         if(this.holder.numChildren > 0)
         {
            while(this.holder.numChildren > 0)
            {
               this.holder.removeChildAt(0);
               _loc5_;
            }
         }
         var _loc1_:* = 0;
         var _loc2_:* = 0;
         _loc5_;
         _loc5_;
         while(true)
         {
            _loc4_;
            _loc5_;
            _loc5_;
            if(_loc2_ >= SafeGlobal.victoryAnimsUnlocked.length)
            {
               break;
            }
            _loc4_;
            _loc4_;
            if(SafeGlobal.victoryAnimsUnlocked[_loc2_])
            {
               _loc5_;
               _loc1_++;
               _loc5_;
               _loc5_;
            }
            _loc2_++;
         }
         this.§_-sD§ = new SelectBoxBackground();
         _loc4_;
         this.§_-sD§.init("VICTORY ANIMATIONS",_loc1_);
         _loc4_;
         this.holder.addChild(this.§_-sD§);
         this.start_x = 0;
         _loc5_;
         this.start_y = 0 - Math.ceil(_loc1_ / 5) * 60;
         _loc4_;
         this.§_-jC§ = new §_-GI§();
         _loc4_;
         _loc5_;
         _loc3_ = -1;
         _loc4_;
         _loc2_ = 0;
         while(_loc2_ < SafeGlobal.victoryAnimsUnlocked.length)
         {
            _loc5_;
            _loc5_;
            if(SafeGlobal.victoryAnimsUnlocked[_loc2_])
            {
               _loc3_++;
               _loc5_;
               _loc5_;
               this.anims.push(new VictoryButton(_loc2_));
               this.holder.addChild(this.anims[this.anims.length - 1]);
               _loc5_;
               _loc5_;
               _loc5_;
               this.anims[this.anims.length - 1].x = this.start_x + _loc3_ % 5 * 60;
               _loc5_;
               this.anims[this.anims.length - 1].y = this.start_y + Math.floor(_loc3_ / 5) * 60;
               _loc5_;
               if(_loc2_ == Global.anim_victory.getInt())
               {
                  _loc4_;
                  _loc4_;
                  _loc5_;
                  _loc5_;
                  _loc4_;
                  _loc4_;
                  _loc5_;
                  _loc5_;
                  _loc4_;
                  _loc4_;
                  this.§_-jC§.x = this.start_x + _loc3_ % 5 * 60;
                  _loc4_;
                  _loc4_;
                  this.§_-jC§.y = this.start_y + Math.floor(_loc3_ / 5) * 60;
               }
            }
            _loc2_++;
            _loc4_;
            _loc4_;
         }
         this.holder.addChild(this.§_-jC§);
      }
      
      public function §_-ez§(param1:int) : *
      {
         var _loc3_:* = false;
         var _loc4_:* = true;
         var _loc2_:* = 0;
         _loc3_;
         LP_TeamSetUp(parent).§_-JH§(param1);
         _loc3_;
         _loc3_;
         _loc2_ = 0;
         while(_loc2_ < this.anims.length)
         {
            if(this.anims[_loc2_].id == param1)
            {
               _loc3_;
               _loc3_;
               _loc3_;
               _loc3_;
               _loc3_;
               _loc3_;
               this.§_-jC§.x = this.§_-3x§(_loc2_);
               _loc3_;
               this.§_-jC§.y = this.§_-7o§(_loc2_);
               _loc3_;
               _loc3_;
               break;
            }
            _loc2_++;
            _loc4_;
            _loc4_;
         }
      }
      
      private function §_-3x§(param1:int) : Number
      {
         var _loc2_:* = true;
         var _loc3_:* = false;
         _loc2_;
         _loc2_;
         _loc2_;
         _loc3_;
         _loc3_;
         return this.start_x + (param1 + 1) % 5 * 60;
      }
      
      private function §_-7o§(param1:int) : Number
      {
         var _loc2_:* = true;
         var _loc3_:* = false;
         _loc3_;
         _loc3_;
         return this.start_y + Math.floor((param1 + 1) / 5) * 60;
      }
      
      public function §_-De§(param1:int) : void
      {
         var _loc2_:* = false;
         var _loc3_:* = true;
         _loc2_;
         this.§_-sD§.§_-b6§(Global.victoryNames[param1]);
      }
      
      public function §_-Rt§() : void
      {
         var _loc1_:* = true;
         var _loc2_:* = false;
         _loc2_;
         _loc2_;
         this.§_-sD§.§_-b6§("VICTORY ANIMATIONS");
      }
   }
}
